...
function getAll(root) {
    ...
    for (var i = 0, len = files.length; i < len; i++) {
        ...
        if (!(/^\.[\w\W]+/.test(cur)) && cur.charAt(0) !== "") {
            ...
            if (stat.isFile()) {
                ext = path.extname(cur);
                res.files.push({
                    parent: root,
                    fullPath: curPath,
                    name: cur,
                    size: div(stat.size, 1024).toFixed(2) + "kb",
                    extName: ext,
                    mimeType: mimeTypes[ext.slice(1)]
                });
            } else if (stat.isDirectory()) {
                res.files.push(getAll(curPath));
                if(!contained(res.files, curPath)) {
                    res.folders.push(curPath);
                }
            }
        }
    }
    res.folders = goRepeat(res.folders);
    return res;
}
 ...
 *                  @attribute      extName     文件结构
 *                  @attribute      mimeType    mime类型        
 *     @attribute  folders      当前目录下所有目录(结构同上)
 * }
 */
function parser(root) {
    ...
    if (all.folders.length) {
        ...
        for (var i = 0, lenI = res.folders.length; i < lenI; i++) {
            res.folders.push(all.folders[i]);
        }
    }
    if (all.files.length) {
